...
class CompileKotlinAgainstCustomBinariesTest : TestCaseWithTmpdir() {
    ...
    /**
     * Compiles all .kt sources under the directory named [libraryName] to [destination].
     * [destination] should be a path to the resulting .js file under [tmpdir].
     *
     * @return the path to the corresponding .meta.js file, e.g. "foo.meta.js" if [destination] is "foo.js"
     */
    private fun compileJsLibrary(
            libraryName: String,
            destination: File = File(tmpdir, "$libraryName.js"),
            additionalOptions: List<String> = emptyList(),
            vararg extraClassPath: File
    ): File {
        val output = compileKotlin(libraryName, destination, extraClassPath.toList(), K2JSCompiler(), additionalOptions, expectedFileName = null)
        assertEquals(normalizeOutput("" to ExitCode.OK), normalizeOutput(output))
        return File(destination.path.replace("\\.js".toRegex(), ".meta.js"))
    }
